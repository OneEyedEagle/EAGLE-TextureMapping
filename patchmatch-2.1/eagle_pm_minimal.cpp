...
void save_bitmap(BITMAP *bmp, const char *filename) {
  // ----- EAGLE added ----- //
  char _rawname[256];
  strcpy(_rawname, filename);
  sprintf(strstr(_rawname, "."), ".txt");
  std::ofstream outfile(_rawname, std::ios::out | std::ios::binary);
...
void patchmatch(BITMAP *a, BITMAP *b, BITMAP *&ann, BITMAP *&annd) {
  ...
  window_w = 0.1 * sqrt(a->w * a->h); // EAGLE added
  for (int ay = 0; ay < aeh; ay++) {
    for (int ax = 0; ax < aew; ax++) {
      // ------ EAGLE added ----- //
      srand( int(time(0)) );
      int bx = MAX(ax-window_w/2, 0) + rand()%window_w;
      bx = MIN(bx, bew);
      int by = MAX(ay-window_w/2, 0) + rand()%window_w;
      by = MIN(by, beh);
      //int bx = rand()%bew;
      //int by = rand()%beh;
      // ------ end of EAGLE----- //
  ...
  for (int iter = 0; iter < pm_iters; iter++) {
    ...
    for (int ay = ystart; ay != yend; ay += ychange) {
      for (int ax = xstart; ax != xend; ax += xchange) {
        ...
        int rs_start = window_w; //rs_max; // EAGLE changed
